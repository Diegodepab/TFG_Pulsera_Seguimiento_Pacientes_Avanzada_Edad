<script>
  ...
  page.subscribe(($page) => {
    ...
    // Elimina la línea que asigna patientId desde la URL
  ...
  onMount(async () => {
    // Maneja el error de permisos con try/catch para evitar que la página falle si el backend da error 500
    let permission;
    try {
      [permission] = await SessionManager.userPermissionsOn([PermissionsEntityType.STUDY]);
    } catch (e) {
      CommonNotifications.genericDanger('Could not load permissions. Please try again later.');
    ...
    // Obtener el patientId del usuario logueado
    ...
    if (currentUser.patientId) {
      ...
      params.set(QueryFields.LIMIT, 1);
      try {
        const searchResults = await patientCtl.search({ params });
        if (searchResults?.items?.length > 0) {
          patientId = searchResults.items[0].id;
        } else {
          patientId = undefined;
        }
      } catch (e) {
  ...
  async function loadStudies() {
    ...
    try {
      ...
      if (!patientId) {
        studies = [];
        throw new Error("No patientId found for current user");
      }
