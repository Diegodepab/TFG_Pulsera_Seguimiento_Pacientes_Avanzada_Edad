...
def bmp_to_header(bmp_file, output_file, array_name):
    ...
    img = img.resize((160, 235))  # Redimensionar la imagen a 160x128 (ajustado a la pantalla)

    # Obtener los datos de píxeles en formato RGB
    ...
    for row in pixel_data:
        for pixel in row:
            ...
            r = (pixel[0] >> 3) & 0x1F  # Red: 5 bits (0-31)
            g = (pixel[1] >> 2) & 0x3F  # Green: 6 bits (0-63)
            b = (pixel[2] >> 3) & 0x1F  # Blue: 5 bits (0-31)
            hex_val = (r << 11) | (g << 5) | b  # Convertir a formato de 16 bits RGB565
