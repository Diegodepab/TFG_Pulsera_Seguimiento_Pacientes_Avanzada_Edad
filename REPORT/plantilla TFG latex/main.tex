...
\section{Diseño de la pulsera (Hardware)}
	...
	\begin{enumerate}
		...
		\begin{itemize}
			...
			%%\item Pulsación simultánea >2 s entra en estado “BOTH”: fondo rojo + publicación MQTT de alerta.  
			%% \item Pulsación de uno solo muestra un aviso amarillo de corta duración (500 ms) para minimizar falsos positivos.  
	...
	\begin{itemize}
		...
		%%\item \textbf>Formateo ISO 8601:} la función \texttt{getISOTime()} devuelve una cadena “YYYY‑MM‑DDThh:mm:ssZ” para incluir en mensajes MQTT.  
...
%%	Para medir frecuencia cardíaca (HR) y saturación de oxígeno (SpO₂):
%%	\begin{itemize}
%%	\item \textbf{Inicialización 400 kHz:} \texttt{particleSensor.begin(Wire,I2C\_SPEED\_FAST)} y \texttt{particleSensor.setup(...)} para modo IR+RED.  
%%	\item \textbf{Máquina de estados:} 
	...
	%%	\item Cálculo de \(\text{zFiltered} = \text{accZ} - \text{media móvil}\).  
	%%	\item Cuando \(\text{zFiltered} > \text{STEP\_THRESHOLD}=1.2\,g\) y han pasado \(\geq 300\,ms\) desde el último paso, se incrementa \texttt{stepCount}.  
	\end{itemize}
	...
	\end{itemize}
%%	\item \textbf{Métricas periódicas:} cada 60 s \texttt{sendMetrics()} publica un JSON con el formato usado en el backend de la plataforma.  
